...
<div class="navbar-container d-flex content">
  ...
  <ul class="nav navbar-nav align-items-center ms-auto">
    ...
    // dd(Auth::user()->submasters);
    echo '<pre>';      

      dd(Auth::user()->getVariantsAttribute());
        foreach (Auth::user()->parent as $key => $value) {
          print_r($value); 
        }
    @endphp --}}
    {{-- {{dd(Auth::user()->submasters())}} --}}
    ...
    <li class="nav-item dropdown dropdown-user">
      <a class="nav-link dropdown-toggle dropdown-user-link" id="dropdown-user" href="javascript:void(0);"
        ...
        <div class="user-nav d-sm-flex d-none">
          ...
          <span class="user-status"> 
            <span class="fw-bolder text-primary px-25">{{ ucwords(Auth::user()->usercode) }}</span>
            {{ ucwords(Auth::user()->getRoleNames()->first()) }}
          </span>
        </div>
        <span class="avatar">
          {!!
          Auth::user() && url_file_exists(URL.USER.Auth::user()->profile_picture)? 
            '<img class="round"
                src="'.URL.USER.Auth::user()->profile_picture.'"
                alt="avatar" height="40" width="40">
            ':
            '<div class="avatar bg-light-primary">
                  <div class="avatar-content">'.getInitials(Auth::user()->name).'</div>
              </div>'
          !!}            
      ...
      <div class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdown-user">
        ...
        <a class="dropdown-item" href="{{ Route::has('panel-settings') ? route('panel-settings',['panel-settings']) : 'javascript:void(0)' }}">
          <i class="me-50" data-feather="settings"></i> Panel Settings
        </a>
        @endif
